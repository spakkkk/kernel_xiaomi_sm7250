...
int fts_wait_tp_to_valid(struct i2c_client *client)
{
	...
	FTS_FUNC_ENTER();
	if (chip_id != 0x54) {
		FTS_ERROR("%s:chip_id = 0x%x", __func__, chip_id);
		chip_id = 0x54;
	}
	do {
		ret = fts_i2c_read_reg(client, FTS_REG_CHIP_ID, &reg_value);
		if ((ret < 0) || (reg_value != chip_id)) {
			FTS_ERROR("TP Not Ready, ReadData = 0x%x ret = %d\n", reg_value, ret);
		} else if (reg_value == chip_id) {
			FTS_ERROR("TP Ready, Device ID = 0x%x", reg_value);
			return 0;
		}
		cnt++;
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
	...
	do {
		...
		if (ret < 0) {
			FTS_ERROR("i2c read invalid,read:0x%02x,i2c_test fail\n", chip_id[0]);
			return ret;
		}
	...
	for (i = 0; i < data->touch_point; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if (!data->fod_finger_skip && fod_overlap_aera == 100) {
				...
				data->fod_down_time++;
				if (data->fod_down_time == 1)
					FTS_INFO("Report_0x152 resume DOWN x:%d y:%d report_area %d success\n", events[i].x, events[i].y, fod_overlap_aera);
			}
			...
			if ((data->touchs & BIT(events[i].id)) == 0)
				FTS_INFO("[B]P%d TOUCH_DOWN!", events[i].id);
			...
			data->touchs &= ~BIT(events[i].id);
			FTS_INFO("[B]P%d TOUCH_UP!", events[i].id);
		}
	}
		...
		case 0x26:
			...
			if (buf[9] == 0) {
				...
				if (!data->fod_finger_skip && data->finger_in_fod) {
					...
					data->fod_down_time++;
					if (data->fod_down_time == 1)
						FTS_INFO("Report_0x152 suspend DOWN x:%d y:%d report_area %d success\n", x, y, fod_overlap_aera);
				}
				mutex_unlock(&data->report_mutex);
			} else {
				...
				data->old_point_id = 0xff;
				data->point_id_changed = false;
				FTS_INFO("Report_0x152 UP for FingerPrint,down times = %d.\n", data->fod_down_time);
				data->fod_down_time = 0;
				fod_overlap_aera = 0;
				if (!data->suspended) {
					FTS_INFO("touch is not in suspend state, report x,y value by touch nomal report\n");
					return -EINVAL;
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
	...
	buf[0] = 0x00;
	ret = fts_i2c_read(data->client, buf, 1, buf, 16);
	if (buf[15] == 0xFF) {
		ret = fts_i2c_read_reg(fts_data->client, FTS_REG_INT_ACK, &reg_value);
		if (ret < 0) {
			FTS_ERROR("read int ack error");
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
	...
	ret =
		request_threaded_irq(ts_data->irq, NULL, fts_ts_interrupt, pdata->irq_gpio_flags | IRQF_ONESHOT | IRQF_PRIME_AFFINE,
...
static void fts_update_touchmode_data(int mode)
{
	...
	FTS_INFO("%s---mode=%d, temp_value=%d\n", __func__, mode, temp_value);
	switch (mode) {
	...
	case Touch_Edge_Filter:
		if (fts_data->gamemode_enabled) {
			...
			if (last_orientation_value == 3) {
				ret = fts_i2c_write_reg(fts_data->client, FTS_REG_EDGE_FILTER_ORIENTATION, 2);
				if (ret < 0)
					FTS_ERROR("write last_orientation_value failed, ret=%d\n", ret);
			}
		}
		break;
					...
					const char *buf, size_t count)
{
	...
	if (value) {
		...
		input_report_abs(input_dev, ABS_MT_WIDTH_MINOR, 100);
