...
extern int32_t nvt_rf_resist_enable(bool enable);
...
int xiaomitouch_register_modedata(struct xiaomi_touch_interface *data)
{
	...
	touch_data->enable_rf_resist = data->enable_rf_resist;
...
static ssize_t xiaomi_rf_resist_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char value = '0';
...
static ssize_t xiaomi_rf_resist_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int command = -1;
	bool enable = false;
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	if (pdata->touch_data->enable_rf_resist)  {
		sscanf(buf, "%d", &command);
		pr_err("[xiaomi] command is: %d", command);
		if (command == 0) {
			enable= false;
			pdata->touch_data->enable_rf_resist(enable);
		} else if (command == 1) {
			enable = true;
			pdata->touch_data->enable_rf_resist(enable);
		} else
...
static DEVICE_ATTR(touch_rf_resist, (S_IRUGO | S_IWUSR | S_IWGRP), xiaomi_rf_resist_show, xiaomi_rf_resist_store);
